...
#define SPI_NAND_CMD_WRSR2       0x31    /* Write status register 2 */
//no      0x35    /* Read status register 2 */
#define SPI_NAND_CMD_RDSR3       0x15    /* Read status register 3 */
#define SPI_NAND_CMD_WRSR3       0x11    /* Write status register 3 */
#define SPI_NAND_CMD_READ        0x03    /* Read data */
#define SPI_NAND_CMD_READ_FAST   0x0B    /* Read data */
#define SPI_NAND_CMD_DREAD       0x3B    /* Read data (1-1-2) */
#define SPI_NAND_CMD_2READ       0xBB    /* Read data (1-2-2) */
#define SPI_NAND_CMD_QREAD       0x6B    /* Read data (1-1-4) */
#define SPI_NAND_CMD_4READ       0xEB    /* Read data (1-4-4) */
...
#define SPI_NAND_CMD_PP_1_1_2    0xA2    /* Dual Page program (1-1-2) */
#define SPI_NAND_CMD_PP_1_1_4    0x32    /* Quad Page program (1-1-4) */
#define SPI_NAND_CMD_PP_1_4_4    0x38    /* Quad Page program (1-4-4) */
#define SPI_NAND_CMD_RDCR        0x15    /* Read control register */
...
#define SPI_NAND_CMD_4BA         0xB7    /* Enter 4-Byte Address Mode */
#define SPI_NAND_CMD_DPD         0xB9    /* Deep Power Down */
#define SPI_NAND_CMD_RDPD        0xAB    /* Release from Deep Power Down */
#define SPI_NAND_CMD_WR_CFGREG2  0x72    /* Write config register 2 */
#define SPI_NAND_CMD_RD_CFGREG2  0x71    /* Read config register 2 */
#define SPI_NAND_CMD_RESET_EN    0x66    /* Reset Enable */
#define SPI_NAND_CMD_RESET_MEM   0x99    /* Reset Memory */
#define SPI_NAND_CMD_BULKE       0x60    /* Bulk Erase */
#define SPI_NAND_CMD_READ_4B      0x13  /* Read data 4 Byte Address */
#define SPI_NAND_CMD_READ_FAST_4B 0x0C  /* Fast Read 4 Byte Address */
#define SPI_NAND_CMD_DREAD_4B     0x3C  /* Read data (1-1-2) 4 Byte Address */
#define SPI_NAND_CMD_2READ_4B     0xBC  /* Read data (1-2-2) 4 Byte Address */
#define SPI_NAND_CMD_QREAD_4B     0x6C  /* Read data (1-1-4) 4 Byte Address */
#define SPI_NAND_CMD_4READ_4B     0xEC  /* Read data (1-4-4) 4 Byte Address */
#define SPI_NAND_CMD_PP_4B        0x12  /* Page Program 4 Byte Address */
#define SPI_NAND_CMD_PP_1_1_4_4B  0x34  /* Quad Page program (1-1-4) 4 Byte Address */
#define SPI_NAND_CMD_PP_1_4_4_4B  0x3e  /* Quad Page program (1-4-4) 4 Byte Address */
 ...
 */
static int spi_nand_send_cmd(const struct device* const dev,
			  ...
			  off_t addr, void* data, size_t length)
{
	...
	// This statement only executes if we want the opcode to also have a specific address.
	if (is_addressed) {
...
#define spi_nor_cmd_read(dev, opcode, dest, length) \
	spi_nand_send_cmd(dev, opcode, 0, 0, dest, length)
#define spi_nor_cmd_addr_read(dev, opcode, addr, dest, length) \
	spi_nand_send_cmd(dev, opcode, NOR_ACCESS_ADDRESSED, addr, dest, length)
#define spi_nor_cmd_write(dev, opcode) \
	spi_nand_send_cmd(dev, opcode, NOR_ACCESS_WRITE, 0, NULL, 0)
#define spi_nor_cmd_addr_write(dev, opcode, addr, src, length) \
	spi_nand_send_cmd(dev, opcode, NOR_ACCESS_WRITE | NOR_ACCESS_ADDRESSED, \
			...
			uint8_t sr)
{
	...
	if (ret == 0) {
		ret = spi_nand_send_cmd(dev, SPI_NOR_CMD_WRSR, NOR_ACCESS_WRITE, 0, &sr,
				     sizeof(sr));
