...
struct stream_metadata {
	...
	uint64_t received_data_messages;
	uint32_t session_id;
...
struct phase_statistics {
	...
	uint64_t raw_nus_bytes;
	uint64_t sensor_bytes;
	uint64_t data_notifications;
	uint32_t max_gap_ms;
	bool has_data;
};
...
static const char *tester_state_name(enum tester_state state)
{
	...
	case TESTER_RECEIVING:
		return "RECEIVING";
	case TESTER_FINISHING:
		return "ECG_FINISHING";
...
static const char *status_name(uint16_t status)
{
	...
	case MSENSE_SENSOR_STREAM_STATUS_STOPPED:
		return "STOPPED";
	...
	case MSENSE_SENSOR_STREAM_STATUS_DISCONNECTED:
		return "DISCONNECTED";
	case MSENSE_SENSOR_STREAM_STATUS_BUFFER_OVERFLOW:
...
static uint64_t elapsed_ms_between(int64_t start_ms, int64_t end_ms)
{
	return end_ms > start_ms ? (uint64_t)(end_ms - start_ms) : 0U;
}

static uint32_t rate_x10(uint64_t value, uint64_t elapsed_ms, uint32_t unit)
{
	if (elapsed_ms == 0U) {
		return 0U;
	}
	/* Diagnostic rates saturate; authoritative stream byte offsets never wrap. */
	uint64_t scaled = value > UINT64_MAX / 10000U ? UINT64_MAX : value * 10000U;

	return MIN(scaled / elapsed_ms / unit, UINT32_MAX);
}
					 ...
					 int64_t received_ms)
{
	...
	if (statistics->has_data) {
		gap_ms = MIN(elapsed_ms_between(statistics->last_data_ms, received_ms), UINT32_MAX);
...
static void stream_statistics_record_data(struct stream_statistics *statistics, uint64_t offset,
					  uint16_t raw_nus_length, uint32_t sensor_bytes,
					  int64_t received_ms)
{
	uint32_t history = offset < MSENSE_SENSOR_STREAM_HISTORY_BYTES ?
		MIN(sensor_bytes, MSENSE_SENSOR_STREAM_HISTORY_BYTES - offset) : 0U;
	uint16_t history_raw = history != 0U ? history + raw_nus_length - sensor_bytes : 0U;

	phase_statistics_record_data(&statistics->total, raw_nus_length, sensor_bytes, received_ms);
	if (history != 0U) {
		phase_statistics_record_data(&statistics->history, history_raw, history, received_ms);
	}
	if (sensor_bytes > history) {
		phase_statistics_record_data(&statistics->forward, raw_nus_length - history_raw,
					     sensor_bytes - history, received_ms);
	}
}
	...
	const struct phase_statistics *statistics, int64_t end_ms)
{
	...
	values.notifications_per_second_x10 =
		rate_x10(statistics->data_notifications, values.elapsed_ms, 1U);
	values.raw_kib_per_second_x10 =
		rate_x10(statistics->raw_nus_bytes, values.elapsed_ms, 1024U);
	values.sensor_kib_per_second_x10 =
		rate_x10(statistics->sensor_bytes, values.elapsed_ms, 1024U);
				    ...
				    int64_t request_start_ms, bool request_started)
{
	...
	uint64_t request_elapsed_ms = request_started ?
		elapsed_ms_between(request_start_ms, statistics->last_data_ms) : 0U;

	post_event("THROUGHPUT_HISTORY id=%u active_elapsed_ms=%llu request_elapsed_ms=%llu "
		   "data_notifs=%llu raw_nus_bytes=%llu sensor_bytes=%llu mean_notif_bytes=%u "
		   "notif_s=%u.%u raw_kib_s=%u.%u sensor_kib_s=%u.%u max_gap_ms=%u",
		   session_id, (unsigned long long)values.elapsed_ms, (unsigned long long)request_elapsed_ms,
		   (unsigned long long)statistics->data_notifications,
		   (unsigned long long)statistics->raw_nus_bytes,
		   (unsigned long long)statistics->sensor_bytes, values.mean_notification_bytes,
				    ...
				    const struct phase_statistics *statistics)
{
	...
	post_event("THROUGHPUT_FORWARD id=%u active_elapsed_ms=%llu data_notifs=%llu "
		   "raw_nus_bytes=%llu sensor_bytes=%llu mean_notif_bytes=%u notif_s=%u.%u "
		   "raw_kib_s=%u.%u sensor_kib_s=%u.%u max_gap_ms=%u",
		   session_id, (unsigned long long)values.elapsed_ms, (unsigned long long)statistics->data_notifications,
		   (unsigned long long)statistics->raw_nus_bytes,
		   (unsigned long long)statistics->sensor_bytes, values.mean_notification_bytes,
		   values.notifications_per_second_x10 / 10U,
		   values.notifications_per_second_x10 % 10U, values.raw_kib_per_second_x10 / 10U,
				    ...
				    const struct phase_statistics *statistics)
{
	...
	post_event("THROUGHPUT id=%u elapsed_ms=%llu data_notifs=%llu raw_nus_bytes=%llu "
		   "sensor_bytes=%llu mean_notif_bytes=%u notif_s=%u.%u raw_kib_s=%u.%u "
		   "sensor_kib_s=%u.%u max_gap_ms=%u",
		   session_id, (unsigned long long)values.elapsed_ms, (unsigned long long)statistics->data_notifications,
		   (unsigned long long)statistics->raw_nus_bytes, (unsigned long long)statistics->sensor_bytes,
				  ...
				  const struct phase_statistics *statistics, int64_t end_ms)
{
	...
	command_printf("THROUGHPUT_LIVE id=%u elapsed_ms=%llu data_notifs=%llu raw_nus_bytes=%llu "
		       "sensor_bytes=%llu mean_notif_bytes=%u notif_s=%u.%u raw_kib_s=%u.%u "
		       "sensor_kib_s=%u.%u max_gap_ms=%u",
		       session_id, (unsigned long long)values.elapsed_ms, (unsigned long long)statistics->data_notifications,
		       (unsigned long long)statistics->raw_nus_bytes, (unsigned long long)statistics->sensor_bytes,
...
{
	uint8_t slot_index = tester.metadata.ecg_filling_slot;

	if (slot_index < ECG_RX_BLOCK_SLOT_COUNT &&
	    atomic_get(&ecg_rx_slots[slot_index].state) == ECG_RX_BLOCK_FILLING &&
	    ecg_rx_slots[slot_index].session_id == tester.metadata.session_id &&
	    ecg_rx_slots[slot_index].capture_generation == tester.capture_generation) {
		atomic_set(&ecg_rx_slots[slot_index].state, ECG_RX_BLOCK_FREE);
	}
	tester.metadata.ecg_filling_slot = ECG_RX_BLOCK_SLOT_NONE;
}

/* tester.lock must be held. FINISHING replaces a separate pending-END flag. */
static void finish_stream_locked(uint16_t status, bool replace_status)
{
	if (tester.state == TESTER_RECEIVING || tester.state == TESTER_FINISHING) {
		if (tester.state == TESTER_RECEIVING || replace_status) {
			tester.end_status = status;
		}
		ecg_release_filling_slot_locked();
		tester.state = TESTER_FINISHING;
		end_completion.session_id = tester.metadata.session_id;
		end_completion.capture_generation = tester.capture_generation;
		if (k_work_submit(&end_completion.work) < 0) {
			tester.state = TESTER_FAILED;
		}
	} else if (tester.state == TESTER_START_PENDING) {
		tester.state = TESTER_FAILED;
	}
}
...
static void mark_protocol_failure(const char *reason)
{
	...
	connection = tester.conn != NULL ? bt_conn_ref(tester.conn) : NULL;
	finish_stream_locked(MSENSE_SENSOR_STREAM_STATUS_INTERNAL_ERROR, true);
	if (tester.state == TESTER_COMPLETE) {
		tester.state = TESTER_FAILED;
	}
	k_spin_unlock(&tester.lock, key);
	post_event("PROTOCOL_ERROR %s", reason);
	stream_progress_stop();
	if (connection != NULL) {
...
static void stream_progress_note(void)
{
	...
	active = tester.peer_device_type != 0U &&
		 (tester.state == TESTER_START_PENDING || tester.state == TESTER_RECEIVING);
	if (active) {
		tester.last_progress_ms = k_uptime_get();
	}
	k_spin_unlock(&tester.lock, key);
	if (active) {
		(void)k_work_reschedule(&stream_progress_work,
					K_MSEC(CONFIG_MSENSE_CENTRAL_NO_PROGRESS_TIMEOUT_MS));
	}
}

static void stream_progress_stop(void)
{
	(void)k_work_cancel_delayable(&stream_progress_work);
}
...
static void stream_progress_work_handler(struct k_work *work)
{
	...
	if (tester.state == TESTER_START_PENDING || tester.state == TESTER_RECEIVING) {
		int64_t elapsed = k_uptime_get() - tester.last_progress_ms;

		if (elapsed >= CONFIG_MSENSE_CENTRAL_NO_PROGRESS_TIMEOUT_MS) {
			timed_out = true;
			session_id = tester.metadata.session_id;
			connection = tester.conn != NULL ? bt_conn_ref(tester.conn) : NULL;
			finish_stream_locked(MSENSE_SENSOR_STREAM_STATUS_INTERNAL_ERROR, true);
		} else if (elapsed >= 0) {
			delay_ms = CONFIG_MSENSE_CENTRAL_NO_PROGRESS_TIMEOUT_MS - elapsed;
		}
	}
	k_spin_unlock(&tester.lock, key);
	if (timed_out) {
		post_event("TIMEOUT receive no progress id=%u", session_id);
		if (connection != NULL) {
			(void)bt_conn_disconnect(connection, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
			bt_conn_unref(connection);
		}
	} else if (delay_ms != 0U) {
		(void)k_work_reschedule(&stream_progress_work, K_MSEC(delay_ms));
	}
}
...
static bool parse_start_ack(const uint8_t *payload, uint8_t device_type,
                            struct stream_metadata *metadata)
{
	uint8_t mode = payload[0];
	uint64_t total = sys_get_le64(&payload[8]);

	if ((device_type != MSENSE_SENSOR_STREAM_DEVICE_ECG &&
	     device_type != MSENSE_SENSOR_STREAM_DEVICE_PPG) ||
	    mode > 1U || payload[1] != MSENSE_SENSOR_STREAM_HISTORY_UNITS ||
	    !all_zero(&payload[2], 6U) ||
	    total != (mode == 0U ? MSENSE_SENSOR_STREAM_FINITE_BYTES : 0U)) {
		return false;
	}
	memset(metadata, 0, sizeof(*metadata));
	metadata->device_type = device_type;
	metadata->record_size = device_type == MSENSE_SENSOR_STREAM_DEVICE_ECG ?
		MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE : MSENSE_SENSOR_STREAM_PPG_RECORD_SIZE;
	metadata->total_sensor_bytes = total;
	metadata->infinity = mode == 1U;
	return true;
}
...
{
	return length >= MSENSE_SENSOR_STREAM_HEADER_BYTES &&
...
static void handle_start_ack(uint32_t session_id, const uint8_t *payload, uint16_t length)
{
	...
	if (tester.state != TESTER_START_PENDING || session_id != tester.metadata.session_id ||
	    length != MSENSE_SENSOR_STREAM_START_ACK_BYTES ||
	    !parse_start_ack(payload, tester.peer_device_type, &metadata) ||
	    metadata.infinity != tester.start_infinity) {
		k_spin_unlock(&tester.lock, key);
		mark_protocol_failure("invalid or unexpected START_ACK");
		return;
	}
	...
	mtu = tester.att_mtu;
	k_spin_unlock(&tester.lock, key);
	stream_progress_note();
	post_event("START_ACK type=%s mode=%s history_blocks=%u total_bytes=%llu mtu=%u",
		   metadata.device_type == MSENSE_SENSOR_STREAM_DEVICE_ECG ? "ECG" : "PPG",
		   metadata.infinity ? "INFINITY" : "FINITE", MSENSE_SENSOR_STREAM_HISTORY_UNITS,
		   (unsigned long long)metadata.total_sensor_bytes, mtu);
	issue_deferred_stop();
}
...
/* tester.lock must be held. */
static bool ecg_validation_pending_locked(uint32_t session_id, uint32_t capture_generation)
{
	uint8_t index;

	for (index = 0U; index < ECG_RX_BLOCK_SLOT_COUNT; index++) {
		if (atomic_get(&ecg_rx_slots[index].state) == ECG_RX_BLOCK_VALIDATING &&
		    ecg_rx_slots[index].session_id == session_id &&
		    ecg_rx_slots[index].capture_generation == capture_generation) {
			return true;
		}
	}
...
/* tester.lock must be held. */
static bool ecg_any_validation_pending_locked(void)
{
	uint8_t index;
...
static bool end_status_is_valid(uint16_t status)
{
	return status == MSENSE_SENSOR_STREAM_STATUS_SUCCESS ||
	       status == MSENSE_SENSOR_STREAM_STATUS_NOT_RECORDING ||
	       status == MSENSE_SENSOR_STREAM_STATUS_STOPPED ||
	       status == MSENSE_SENSOR_STREAM_STATUS_STORAGE_ERROR ||
	       status == MSENSE_SENSOR_STREAM_STATUS_INTERNAL_ERROR ||
	       status == MSENSE_SENSOR_STREAM_STATUS_DISCONNECTED ||
	       status == MSENSE_SENSOR_STREAM_STATUS_BUFFER_OVERFLOW;
}

static void post_stream_prefix(uint8_t device_type, uint32_t session_id, uint64_t records,
			       uint64_t skipped_history_blocks)
{
	if (device_type == MSENSE_SENSOR_STREAM_DEVICE_ECG) {
		post_event("ECG_PREFIX id=%u valid_blocks=%llu valid_bytes=%llu "
			   "skipped_history_blocks=%llu", session_id,
			   (unsigned long long)records,
			   (unsigned long long)(records * MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE),
			   (unsigned long long)skipped_history_blocks);
	} else {
		post_event("PPG_PREFIX id=%u complete_records=%llu complete_bytes=%llu", session_id,
			   (unsigned long long)records,
			   (unsigned long long)(records * MSENSE_SENSOR_STREAM_PPG_RECORD_SIZE));
	}
}
...
static void complete_pending_end(uint32_t session_id, uint32_t capture_generation)
{
	...
	if (tester.state != TESTER_FINISHING || session_id != tester.metadata.session_id ||
	    capture_generation != tester.capture_generation ||
	    ecg_validation_pending_locked(session_id, capture_generation)) {
		k_spin_unlock(&tester.lock, key);
		return;
	}
	metadata = tester.metadata;
	statistics = tester.statistics;
	status = tester.end_status;
	if (metadata.device_type == MSENSE_SENSOR_STREAM_DEVICE_ECG) {
		uint64_t complete_blocks = metadata.expected_byte_offset / metadata.record_size;

		complete_records = metadata.skipped_history_blocks <= complete_blocks &&
			metadata.validated_records ==
			complete_blocks - metadata.skipped_history_blocks;
	} else {
		complete_records = metadata.validated_records ==
			metadata.expected_byte_offset / metadata.record_size;
	}
	success = status == MSENSE_SENSOR_STREAM_STATUS_SUCCESS && !metadata.infinity &&
		  metadata.expected_byte_offset == metadata.total_sensor_bytes &&
		  complete_records;
	clean_stop = (status == MSENSE_SENSOR_STREAM_STATUS_STOPPED ||
		      status == MSENSE_SENSOR_STREAM_STATUS_NOT_RECORDING) &&
		     complete_records;
	reconnect = tester.reconnect_after_stream && tester.conn == NULL;
	tester.reconnect_after_stream = false;
	tester.state = reconnect ? TESTER_IDLE :
		(success || clean_stop ? TESTER_COMPLETE : TESTER_FAILED);
	k_spin_unlock(&tester.lock, key);

	if (!statistics.history_reported && statistics.history.has_data) {
		post_throughput_history(session_id, &statistics.history, statistics.request_start_ms,
					statistics.request_started);
	}
	post_stream_prefix(metadata.device_type, session_id, metadata.validated_records,
			   metadata.skipped_history_blocks);
	if (success) {
		post_throughput_forward(session_id, &statistics.forward);
		post_throughput_summary(session_id, &statistics.total);
		post_event("STREAM_OK id=%u bytes=%llu data_messages=%llu", session_id,
			   (unsigned long long)metadata.expected_byte_offset,
			   (unsigned long long)metadata.received_data_messages);
	} else {
		post_event("STREAM_END status=%s(0x%04x) bytes=%llu data=%llu", status_name(status),
			   status, (unsigned long long)metadata.expected_byte_offset,
			   (unsigned long long)metadata.received_data_messages);
	}
	if (reconnect) {
...
static void ecg_block_validate_work_handler(struct k_work *work)
{
	...
	uint64_t skipped_history_blocks;
	int ret;
	bool history_padding;
	bool failed = false;
	struct bt_conn *connection = NULL;
	k_spinlock_key_t key;

	history_padding = slot->byte_offset <=
		MSENSE_SENSOR_STREAM_HISTORY_BYTES - MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE &&
		all_zero(slot->data, MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE);
	ret = history_padding ? 0 : msense_ecg_block_validate(slot->data, &block);
	key = k_spin_lock(&tester.lock);
	if (atomic_get(&slot->state) != ECG_RX_BLOCK_VALIDATING ||
	    (tester.state != TESTER_RECEIVING && tester.state != TESTER_FINISHING) ||
	    slot->session_id != tester.metadata.session_id ||
	    slot->capture_generation != tester.capture_generation) {
		atomic_set(&slot->state, ECG_RX_BLOCK_FREE);
		k_spin_unlock(&tester.lock, key);
		return;
	}
	if (history_padding && tester.metadata.ecg_previous_block_valid) {
		ret = -EINVAL;
	}
	if (ret == 0 && !history_padding &&
	    tester.metadata.ecg_previous_block_valid) {
	...
	if (ret != 0) {
		...
		failed = true;
	} else if (history_padding) {
		tester.metadata.skipped_history_blocks++;
	} else {
		tester.metadata.ecg_previous_block = block;
		tester.metadata.ecg_previous_block_valid = true;
		tester.metadata.validated_records++;
	}
	session_id = slot->session_id;
	capture_generation = slot->capture_generation;
	validated_blocks = tester.metadata.validated_records;
	skipped_history_blocks = tester.metadata.skipped_history_blocks;
	atomic_set(&slot->state, ECG_RX_BLOCK_FREE);
	k_spin_unlock(&tester.lock, key);

	post_stream_prefix(MSENSE_SENSOR_STREAM_DEVICE_ECG, session_id, validated_blocks,
			   skipped_history_blocks);
	if (failed) {
		post_event("PROTOCOL_ERROR ECB2 validation failed (%d)", ret);
		stream_progress_stop();
		if (connection != NULL) {
			(void)bt_conn_disconnect(connection, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
			bt_conn_unref(connection);
	...
	complete_pending_end(session_id, capture_generation);
}
...
static void end_complete_work_handler(struct k_work *work)
{
	...
	ARG_UNUSED(work);
	key = k_spin_lock(&tester.lock);
	session_id = end_completion.session_id;
	capture_generation = end_completion.capture_generation;
	k_spin_unlock(&tester.lock, key);
	complete_pending_end(session_id, capture_generation);
}
...
static void ecg_rx_initialize(void)
{
	...
	k_work_init(&end_completion.work, end_complete_work_handler);
}
			       ...
			       uint16_t raw_nus_length, int64_t received_ms)
{
	...
	uint64_t offset;
	const uint64_t history_bytes = MSENSE_SENSOR_STREAM_HISTORY_BYTES;
	size_t payload_offset = MSENSE_SENSOR_STREAM_DATA_PREFIX_BYTES;
	size_t remaining;
	...
	offset = sys_get_le64(&payload[0]);
	remaining = length - MSENSE_SENSOR_STREAM_DATA_PREFIX_BYTES;

	key = k_spin_lock(&tester.lock);
	if (tester.state != TESTER_RECEIVING ||
	    session_id != tester.metadata.session_id ||
	    offset != tester.metadata.expected_byte_offset ||
	    remaining > UINT64_MAX - offset ||
	    (!tester.metadata.infinity &&
	     (offset > tester.metadata.total_sensor_bytes ||
	      remaining > tester.metadata.total_sensor_bytes - offset))) {
		k_spin_unlock(&tester.lock, key);
		mark_protocol_failure("DATA offset/length");
		return;
	}

	if (tester.metadata.device_type == MSENSE_SENSOR_STREAM_DEVICE_PPG) {
		/* Existing PPG records have no CRC: accept only the complete framed prefix. */
		tester.metadata.expected_byte_offset += remaining;
		tester.metadata.validated_records = tester.metadata.expected_byte_offset /
			MSENSE_SENSOR_STREAM_PPG_RECORD_SIZE;
		remaining = 0U;
	}
	while (remaining > 0U) {
		size_t copy_bytes;
		...
		if (tester.metadata.ecg_filling_slot == ECG_RX_BLOCK_SLOT_NONE) {
			...
			block_offset = tester.metadata.expected_byte_offset %
				MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE;
			ecg_rx_slots[slot_index].byte_offset =
				tester.metadata.expected_byte_offset - block_offset;
		} else {
			slot_index = tester.metadata.ecg_filling_slot;
			if (atomic_get(&ecg_rx_slots[slot_index].state) != ECG_RX_BLOCK_FILLING) {
				k_spin_unlock(&tester.lock, key);
				mark_protocol_failure("ECG DATA filling slot");
				return;
			}
		}
		...
		block_offset = tester.metadata.expected_byte_offset %
			       MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE;
		copy_bytes = MIN(remaining,
				 (size_t)MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE - block_offset);
		memcpy(&ecg_rx_slots[slot_index].data[block_offset], &payload[payload_offset],
		       copy_bytes);
		payload_offset += copy_bytes;
		remaining -= copy_bytes;
		tester.metadata.expected_byte_offset += copy_bytes;
		if (block_offset + copy_bytes == MSENSE_SENSOR_STREAM_ECG_RECORD_SIZE) {
			tester.metadata.ecg_filling_slot = ECG_RX_BLOCK_SLOT_NONE;
			atomic_set(&ecg_rx_slots[slot_index].state, ECG_RX_BLOCK_VALIDATING);
			if (k_work_submit(&ecg_rx_slots[slot_index].work) < 0) {
				atomic_set(&ecg_rx_slots[slot_index].state, ECG_RX_BLOCK_FREE);
				k_spin_unlock(&tester.lock, key);
	...
	stream_statistics_record_data(&tester.statistics, offset, raw_nus_length,
			      length - MSENSE_SENSOR_STREAM_DATA_PREFIX_BYTES, received_ms);
	if (offset < history_bytes && tester.metadata.expected_byte_offset >= history_bytes &&
	    !tester.statistics.history_reported) {
...
static bool result_status_is_valid(uint16_t status)
{
	return (status >= MSENSE_SENSOR_STREAM_STATUS_NOT_RECORDING &&
		status <= MSENSE_SENSOR_STREAM_STATUS_UNSUPPORTED_VERSION) ||
...
static void handle_result(uint32_t session_id, const uint8_t *payload, uint16_t length)
{
	...
	if (length != MSENSE_SENSOR_STREAM_END_BYTES) {
		mark_protocol_failure("invalid RESULT");
		return;
	}
	status = sys_get_le16(&payload[0]);
	if (!result_status_is_valid(status)) {
	...
	if (session_id != tester.last_command_session_id) {
		k_spin_unlock(&tester.lock, key);
		post_event("IGNORED RESULT id=%u", session_id);
		return;
	}
	if (tester.state == TESTER_START_PENDING) {
		...
		tester.command_pending = false;
		rejected_start = true;
	} else if (opcode != MSENSE_SENSOR_STREAM_OPCODE_STOP ||
		   tester.state != TESTER_RECEIVING) {
		k_spin_unlock(&tester.lock, key);
	...
	post_event(rejected_start ? "START_RESULT status=%s(0x%04x)" :
		   "STOP_RESULT status=%s(0x%04x)", status_name(status), status);
	if (rejected_start) {
		stream_progress_stop();
...
static void handle_end(uint32_t session_id, const uint8_t *payload, uint16_t length)
{
	...
	status = sys_get_le16(&payload[0]);
	if (!end_status_is_valid(status)) {
		mark_protocol_failure("invalid END status");
		return;
	}
	...
	/* Raw END is enqueued by the callback before requesting relay close. */
	atomic_set(&relay_close_after_notification, 1);
	tester.command_pending = false;
	finish_stream_locked(status, false);
	k_spin_unlock(&tester.lock, key);
	stream_progress_stop();
}
				...
				const void *data, uint16_t length)
{
	...
	if (data == NULL) {
		handle_subscription_removed(conn);
	...
	/* Control messages settle the stream before their diagnostic relay copy. */
			       ...
			       struct bt_gatt_subscribe_params *params)
{
	...
	key = k_spin_lock(&tester.lock);
	tester.att_mtu = mtu;
	peer_device_type = tester.peer_device_type;
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	if (tester.conn == conn) {
		...
		memset(&tester.link, 0, sizeof(tester.link));
		if (tester.state == TESTER_RECEIVING || tester.state == TESTER_FINISHING) {
			tester.reconnect_after_stream =
				tester.reconnect_enabled && tester.peer_address_valid;
			finish_stream_locked(MSENSE_SENSOR_STREAM_STATUS_DISCONNECTED, false);
		} else {
			ecg_release_filling_slot_locked();
			tester.state = TESTER_IDLE;
...
static uint8_t device_type_for_name(const char *name)
{
	if (strncmp(name, "MSense4PPG-", 11U) == 0) {
		return MSENSE_SENSOR_STREAM_DEVICE_PPG;
	}
	if (strncmp(name, "MSense4ECG-", 11U) == 0) {
		return MSENSE_SENSOR_STREAM_DEVICE_ECG;
	}
	return 0U;
}
			 ...
			 struct net_buf_simple *ad)
{
	...
	device_type = device_type_for_name(name.value);
	...
	if (target == SCAN_TARGET_LIST) {
		...
		if (!tester.peer_address_valid ||
		    bt_addr_le_cmp(address, &tester.peer_address) != 0 || device_type == 0U) {
			k_spin_unlock(&tester.lock, key);
			return;
		}
...
static int issue_nus_command(uint8_t opcode, uint32_t session_id)
{
	...
	key = k_spin_lock(&tester.lock);
	if (tester.write_pending || tester.command_pending || !tester.subscribed ||
	    nus_client.handles.rx == 0U) {
		k_spin_unlock(&tester.lock, key);
		bt_conn_unref(connection);
		return -EBUSY;
	}
	protocol_version = MSENSE_SENSOR_STREAM_PROTOCOL_VERSION;
	if (tester.peer_device_type == 0U) {
		k_spin_unlock(&tester.lock, key);
		bt_conn_unref(connection);
	...
	if (opcode == MSENSE_SENSOR_STREAM_OPCODE_START ||
	    opcode == MSENSE_SENSOR_STREAM_OPCODE_START_INFINITY) {
...
static void issue_deferred_stop(void)
{
	...
	if (!tester.stop_after_start_ack || tester.state != TESTER_RECEIVING ||
	    tester.write_pending || tester.command_pending) {
		k_spin_unlock(&tester.lock, key);
		return;
	}
	session_id = tester.metadata.session_id;
	...
	err = issue_nus_command(MSENSE_SENSOR_STREAM_OPCODE_STOP, session_id);
	if (err != 0) {
		mark_protocol_failure("deferred STOP write failed");
	} else {
		post_event("STOP_SENT id=%u deferred=1", session_id);
	}
}
...
static void begin_start(uint32_t requested_session_id, bool has_requested_id, bool infinity)
{
	struct bt_conn *connection;
	uint32_t session_id;
	int err;
	...
	if ((tester.state != TESTER_READY && tester.state != TESTER_COMPLETE) ||
	    !tester.subscribed || tester.write_pending || tester.command_pending ||
	    atomic_get(&binary_port_mode) != BINARY_PORT_IDLE ||
	    ecg_any_validation_pending_locked()) {
	...
	if (tester.next_session_id == 0U) {
		...
		command_printf("ERR session ID space exhausted; reconnecting");
		return;
	}
	if (has_requested_id && requested_session_id < tester.next_session_id) {
		relay_request_idle();
		k_spin_unlock(&tester.lock, key);
		command_printf("ERR start ID must be fresh for this connection");
		return;
	}
	session_id = has_requested_id ? requested_session_id : tester.next_session_id;
	tester.next_session_id = session_id + 1U;
	...
	tester.metadata.ecg_filling_slot = ECG_RX_BLOCK_SLOT_NONE;
	tester.start_infinity = infinity;
	tester.stop_after_start_ack = false;
	tester.state = TESTER_START_PENDING;
	k_spin_unlock(&tester.lock, key);

	err = issue_nus_command(infinity ? MSENSE_SENSOR_STREAM_OPCODE_START_INFINITY :
					 MSENSE_SENSOR_STREAM_OPCODE_START, session_id);
	if (err != 0) {
		key = k_spin_lock(&tester.lock);
		if (tester.state == TESTER_START_PENDING) {
			tester.state = TESTER_READY;
	...
	stream_progress_note();
	command_printf("START_SENT id=%u mode=%s", session_id,
		       infinity ? "INFINITY" : "FINITE");
}

static void begin_stop(uint32_t requested_session_id, bool has_requested_id)
{
	uint32_t session_id;
	int err;
	k_spinlock_key_t key = k_spin_lock(&tester.lock);

	if (tester.state != TESTER_RECEIVING && tester.state != TESTER_START_PENDING) {
		enum tester_state state = tester.state;
		k_spin_unlock(&tester.lock, key);
		command_printf("ERR stop requires an active stream and completed write (state=%s)",
			       tester_state_name(state));
		return;
	}
	session_id = has_requested_id ? requested_session_id : tester.metadata.session_id;
	if (session_id != tester.metadata.session_id) {
		k_spin_unlock(&tester.lock, key);
		command_printf("ERR stop ID must match active stream");
		return;
	}
	if (tester.state == TESTER_START_PENDING) {
		if (tester.stop_after_start_ack) {
			k_spin_unlock(&tester.lock, key);
			command_printf("ERR stop already deferred");
			return;
		}
		tester.stop_after_start_ack = true;
		k_spin_unlock(&tester.lock, key);
		command_printf("STOP_DEFERRED id=%u", session_id);
		return;
	}
	if (tester.command_pending) {
		k_spin_unlock(&tester.lock, key);
		command_printf("ERR stop command already pending");
		return;
	}
	if (tester.write_pending) {
		k_spin_unlock(&tester.lock, key);
		command_printf("ERR stop requires completed write");
		return;
	}
	k_spin_unlock(&tester.lock, key);
	err = issue_nus_command(MSENSE_SENSOR_STREAM_OPCODE_STOP, session_id);
	if (err != 0) {
		command_printf("ERR STOP write %d", err);
		return;
	}
	command_printf("STOP_SENT id=%u", session_id);
}
...
static void print_status(void)
{
	...
	command_printf("STATUS state=%s subscribed=%u smp=%u mtu=%u id=%u bytes=%llu data=%llu "
		       ...
		       (unsigned long long)metadata.received_data_messages,
		       (unsigned long long)metadata.expected_byte_offset,
		       (unsigned long long)(metadata.record_size ? metadata.expected_byte_offset /
...
static void handle_command(struct command_line *line)
{
	...
	if (strcmp(command, "collect") == 0 && extra == NULL) {
		bool enable;
		enum tester_state state;
		uint8_t device_type;
		int err;
		...
		if (argument == NULL ||
		    (strcmp(argument, "on") != 0 && strcmp(argument, "off") != 0)) {
			command_printf("ERR usage: collect on|off");
			return;
		}
		enable = strcmp(argument, "on") == 0;
		key = k_spin_lock(&tester.lock);
		state = tester.state;
		...
		if ((enable && state != TESTER_READY && state != TESTER_COMPLETE) ||
		    (!enable && state != TESTER_READY && state != TESTER_COMPLETE &&
		     state != TESTER_RECEIVING)) {
			command_printf("ERR collect %s unavailable in state=%s", argument,
				       tester_state_name(state));
			return;
		}
	...
	if (strcmp(command, "start") == 0) {
		bool infinity = argument != NULL && strcmp(argument, "infinity") == 0;
		char *id_text = infinity ? extra : argument;

		if ((!infinity && extra != NULL) ||
		    (id_text != NULL && (!parse_u32(id_text, &value) || value == 0U))) {
			command_printf("ERR usage: start [infinity] [nonzero-id]");
			return;
		}
		begin_start(value, id_text != NULL, infinity);
		return;
	}
	if ((strcmp(command, "stop") == 0 || strcmp(command, "cancel") == 0) && extra == NULL) {
		if (argument != NULL && (!parse_u32(argument, &value) || value == 0U)) {
			command_printf("ERR stop ID must be a nonzero decimal or 0x hexadecimal uint32");
			return;
		}
		begin_stop(value, argument != NULL);
		return;
	}
...
int main(void)
{
	...
	command_printf("MSENSE_CENTRAL_READY nus_protocol=3 dfu_protocol=1; use help");
	while (true) {
