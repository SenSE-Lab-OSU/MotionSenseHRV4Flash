...
def main():
    ...
    metadata_ok = re.search(
        r"START_ACK type=ECG[^\r\n]*mode=FINITE history_blocks=8 total_bytes=131072",
        response,
    )
    ...
    if not metadata_ok:
        print("TEST_FAIL ECG v0 geometry was not reported")
        return 5
    if not stream_ok or frames == 0 or payload_bytes == 0:
        print("TEST_FAIL complete validated 128 KiB relay was not observed")
        return 6
    print("TEST_PASS ECGv0 geometry, ECB2 validation, and 128 KiB relay completed")
    return 0
