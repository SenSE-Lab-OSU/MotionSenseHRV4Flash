...
- Supply the exact MCUboot PPG application `.bin`, Central J-Link serial and its
  ...
  stream IDs, DFU transaction, and a new evidence directory. Historical
...
recording files remain byte-identical.
...
and `summary.json` result `PASS`. Optional reset persistence must pass when
