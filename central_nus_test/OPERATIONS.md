...
| `NUS_UNSOLICITED_IGNORED` | The peer sent NUS data while no commanded stream owned the binary relay; the Central first such notification is reported and safely discarded. |
...
| `PEER_READY nus=<0|1> smp=<0|1> legacy=<0|1> peer_name=<name> peer_addr=<address> peer_addr_type=<type>` | Final discovery availability and machine-parseable advertised identity for the peer. |
| `COLLECT_READY`, `COLLECT_SENT`, `COLLECT_RESULT` | Collection-control discovery and acknowledged write state for PPG or ECG. |
| `LEGACY_RESET_READY`, `REMOTE_STATUS_READY`, `LEGACY_HANDLES` | Legacy control/status handle discovery. Zero-valued handles are unavailable. |
...
| `ECG_PREFIX` | Complete ECB2 blocks validated so far; it remains meaningful if a later END, disconnect, or validation fault fails the stream. |
...
| `STREAM_END` | Stream terminal status; `STOPPED` and `NOT_RECORDING` can be normal bounded endings, while other statuses identify the termination reason. |
| `PROTOCOL_ERROR`, `ERROR ...` | Invalid or failed capture. |
| `RELAY_IDLE` | Binary relay has completely drained and can be handed to DFU. |

The PPG stream and finite ECG stream each carry 131,072 bytes. Shared v0 START_ACK
is 16 bytes (`FINITE`/`INFINITY`, eight 4096-byte history units, and finite total bytes),
DATA carries a uint64 byte offset followed by arbitrary sensor-byte fragments, and END
is its uint16 status. The receiver validates each complete ECB2 block before
announcing it in `ECG_PREFIX`; raw relay frames can contain partial fragments,
so use `ECG_PREFIX` as the complete validated-output boundary. Relay frames are
little-endian `MRLY` v1 records: magic (4), version (1), type (1), raw NUS
length (u16), relay sequence (u32), then exact raw NUS bytes. They do not
include command text or delimiters.
