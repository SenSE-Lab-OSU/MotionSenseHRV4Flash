 ...
 *  2. Disable all interrupts (EN_INT = 0) so no stale sources fire during
 *     reconfiguration.
 ...
 *  6. Set the FIFO interrupt threshold (EFIT) to 8 samples (MNGR_INT), so
 *     INTB asserts once at least 8 samples are queued.
 ...
 *
 * Arms the ECG FIFO interrupt sources on the INTB pin (EN_INT) and then
 * issues a SYNCH command, which resets the FIFO and the internal sample
 * clock so acquisition starts from a clean, aligned state. After this call
 * the INTB line will assert whenever the FIFO reaches the threshold
 * configured by max30001_ecg_init_512(), signaling the application to drain
 * samples. Must be called after max30001_ecg_init_512().
 *
 ...
 *
 * Tears down acquisition in three steps: mask all interrupt sources
 * (EN_INT = 0), disable the ECG channel and lead bias (CNFG_GEN = 0), and
 * reset the FIFO to discard any samples still queued. Unlike the other
