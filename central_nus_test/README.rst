...
This is a deliberately small, temporary tester for the MotionSense shared
version-0 PPG and ECG stream. It runs on an nRF5340 DK or
nRF54L15 DK as a Bluetooth LE Central and connects to either a PPG or ECG
peripheral. It discovers standard Nordic UART Service (NUS), validates the
protocol, and relays received NUS notifications unchanged to a second
interface-MCU virtual COM port. PPG and finite ECG each contain 131,072 sensor
bytes; both support an unbounded ``infinity`` session.
...
formats storage and reboots, ``reset 120`` requests a non-destructive,
storage-aware PPG reboot, ``reset 121`` requests an emergency immediate reset,
and ``reset 132`` clears the persisted bad-block table before reset. Reset
writes use an ATT write with response; the command port separately reports
write acceptance, disconnect, advertising, reconnection, and handle
rediscovery. Reconnection is bounded to five minutes. Before PPG ``reset 120``,
stop streaming, send ``collect off``, and confirm remote status bytes 1 and 2
are both zero.
...
rejected starts; reconnect before reusing one. A ``stop`` is allowed after
its START write completes, including while the START result is pending, and
ends at the submitted record boundary. ``cancel`` remains an alias for ``stop``
for existing command scripts. PPG now uses the v0 transport, so old v1 wire
decoders must be updated; its 16-byte records and files are unchanged.
...
.. code-block:: text
   ...
   START_SENT id=1
   START_ACK type=PPG mode=FINITE history_blocks=8 total_bytes=131072 mtu=...
   START_ACK type=ECG mode=FINITE history_blocks=8 total_bytes=131072 mtu=...
   ECG_PREFIX id=1 valid_blocks=32 valid_bytes=131072
...
1. Start recording on the PPG or ECG device and wait for its fresh history
   window (at least 8 seconds for PPG or about 21.21875 seconds for ECG).
...
3. Send ``connect ppg`` or ``connect ecg``. Wait for ``NUS_READY`` and an ATT
   MTU of at least 128.
4. Send ``start`` (or ``start infinity`` for either sensor). Keep the relay reader
   running through the frame containing the raw NUS END message. ``STREAM_OK``
   means the tester's in-device checks passed, but it can be printed before the
   relay thread has physically written every queued frame to the VCOM UART.
5. Independently parse the relay frames and verify the raw stream against disk
   records. The tester checks common v0 framing and 64-bit byte offsets for both
   sensors. It additionally validates ECG CRC/reserved bytes/sample tags and
   adjacent-block continuity. PPG records retain their existing format without
   a CRC; ``PPG_PREFIX`` reports complete framed records, not CRC validation. ``ECG_PREFIX`` reports the complete validated prefix separately
   from a later stream termination.

The peripheral must acquire the 96 KiB future portion before a finite stream
can finish: roughly 24 seconds for PPG and 63.65625 seconds for ECG, plus BLE
drain time. ``infinity`` has no overall acquisition duration for either sensor.
...
* It validates protocol v0 for both products. DATA may split or join records.
  Malformed framing, offsets, queue exhaustion, or no-progress timeout terminate
  the connection. ECG additionally checks ECB2 CRC/continuity. Complete accepted
  prefix records survive termination; a partial tail is discarded.
* STOP completes the already-started record: 16 bytes for PPG, 4096 for ECG.
  ACK history_blocks=8 means eight 4096-byte units (32 KiB) for either product,
  not eight PPG samples. FINITE retains the existing 8-second PPG history and
  24-second future portion; INFINITY recycles that same bounded future buffer.
