# ECGv0 installation/build note (Phase 5 draft)
...
in the repository README with `ECGv0` and `ecgv0/nrf5340/cpuapp`. Do not use a
...
3. Apply the project-specific local SDK changes documented in `changelist.txt` to the v2.9.3 SDK checkout. The repository provides `msense_ncs_v2.9.3_lfxo_bypass.patch` for only the LFXO bypass change; it is not a complete NCS v2.9.3 patch.
4. Using visual studio, click on the nrf connect tab. Make sure you've selected the v2.9.3 sdk. If everything has installed properly and the SDK is chosen, you should see a button that says 'Open an Existing Application' within the 'Welcome' Tab. After pressing this, navigate to the repository folder `ECGv0` with the opened file explorer.
...
6. In the build configuration, select `ecgv0/nrf5340/cpuapp` as the board target. Use `prj.conf` as the application configuration file; the repository board root is supplied by the application and sysbuild CMake files.
